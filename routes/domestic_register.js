...
router.get('/domestic_details', ensureAuthenticated, function(req, res){
somast.find({co_code:req.session.compid,div_code:req.session.divid,main_bk:"domesticSales"}, function (err,somast){
   ...
   if (err) {
       ...
       res.render('domestic_details.hbs', {
           pageTitle:'Domestic Details',
           somast: somast,
           acmast: acmast,
           product: product,
           workmast:workmast,
...
router.get('/domestic_summary', ensureAuthenticated, function(req, res){
    somast.find({co_code:req.session.compid,div_code:req.session.divid,main_bk:"domesticSales"}, function (err,somast){
        acmast.find({main_bk:"customer"}, function (err, acmast){
            product.find({masterid:req.session.masterid}, function (err, product){
                workmast.find({co_code:req.session.compid,div_code:req.session.divid}, function (err, workmast){
       if (err) {
           ...
           res.render('domestic_summary.hbs', {
               pageTitle:'Domestic Summary',
               somast: somast,
               acmast: acmast,
               product: product,
               workmast:workmast,
...
router.get('/domestic_register', ensureAuthenticated, function(req, res){
             party.find({masterid:req.session.masterid}, function (err, party){
            if (err) {
                console.log(err);
            } else {
...
function CreateHTML(somast,buy_cus_name,req){
    return new Promise(function(fullfill){
        ...
        console.log("buy_cus_name"+buy_cus_name);
            ...
            html+='<tr>';
            html+='<th>Control</th>';
            html+='<th>Domestic Date</th>';
            html+='<th>Domestic No</th>';
            ...
            for (let i = 0; i <somast.length; i++) {
               ...
               // console.log(somastdet)
                ...
                html += '<tr>';
                html += '<td class="domestic-control" data-child="'+ encodeURI(JSON.stringify(childarray))+'"><i class="fa fa-plus-square handicon"></i> </td>';
...
function CreateHTML1(somast,buy_cus_name,req){
    return new Promise(function(fullfill){
      ...
      //  console.log("buy_cus_name"+buy_cus_name);
            ...
            html+='<thead>';
            html+='<tr>';
            html+='<th>Domestic</br> Date</th>';
            html+='<th>Domestic</br> No</th>';
...
router.post('/domesticdetails', ensureAuthenticated, function(req, res){
       var start_date= req.body.start_date;
        ...
        var qry = "";
        if (buy_cus_name=="") qry = {$and: [{main_bk:"domesticSales"},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
        if (buy_cus_name!="") qry = {$and: [{main_bk:"domesticSales"},{buy_cus_name:buy_cus_name},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
...
router.post('/domesticsummary', ensureAuthenticated, function(req, res){
    var start_date= req.body.start_date;
     var end_date= req.body.end_date;
     var  buy_cus_name=req.body.buy_cus_name;
     const strtdate = moment(start_date, "DD/MM/YYYY").tz("Asia/Kolkata").startOf('day').format('x');
