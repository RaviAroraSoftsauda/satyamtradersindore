...
router.get('/salesorder_details', ensureAuthenticated, function(req, res){
somast.find({co_code:req.session.compid,div_code:req.session.divid,main_bk:"salesorder"}, function (err,somast){
   ...
   if (err) {
       ...
       res.render('salesorder_details.hbs', {
...
router.get('/sales_order_summary', ensureAuthenticated, function(req, res){
    somast.find({co_code:req.session.compid,div_code:req.session.divid,main_bk:"salesorder"}, function (err,somast){
        acmast.find({main_bk:"customer"}, function (err, acmast){
            product.find({masterid:req.session.masterid}, function (err, product){
                workmast.find({co_code:req.session.compid,div_code:req.session.divid}, function (err, workmast){
       if (err) {
           ...
           res.render('sales_order_summary.hbs', {
               pageTitle:'Sales Order Summary',
               somast: somast,
               acmast: acmast,
               product: product,
               workmast:workmast,
...
router.get('/salesorder_register', ensureAuthenticated, function(req, res){
             party.find({masterid:req.session.masterid}, function (err, party){
            if (err) {
                console.log(err);
            } else {
...
function CreateHTML(somast,qtybal,buy_cus_name,req){
            ...
            for (let i = 0; i <somast.length; i++) {
                ...
                if(somast[i]!=null) var somastdet = somast[i]['sales_or_group'];
                    ...
                    for (let j = 0; j < somastdet.length; j++) {
                        var printallw = true;
                        if ((qtybal=="true") && parseInt(somastdet[j]['qty_bal'])<=0) printallw=false;
                        if (printallw ==true)
                        {  
                          
                    if (somastdet[j]['so_disc'] != null) var so_disc= somastdet[j]['so_disc']['prdt_desc'];
                    else var so_disc = '';
                    if (somastdet[j]['order_price'] != undefined) var order_price= somastdet[j]['order_price'];
                    else var order_price = '';
                    if (somastdet[j]['sopacper_carten'] != undefined) var sopacper_carten= somastdet[j]['sopacper_carten'];
                    else var sopacper_carten = '';
                    ...
                    if (somastdet[j]['qty_bal'] != undefined) var qty_bal= somastdet[j]['qty_bal'];
                    else var qty_bal = '';
                        
                    var obj = {"ItemName":so_disc,"OrderPrice":order_price,"PackPerCarten": sopacper_carten,"Qty": so_qty,"Discount": so_discount,"Valuet": total_amt,"QtyBal":qty_bal};
                    childarray.push(obj);  
...
function CreateHTML1(somast,buy_cus_name,qty_bal,req){
    return new Promise(function(fullfill){
        var html='<div class="showsearchdata">';
        html+='<div class="row">';
        html+='<div class="col-md-12">';
      ...
      // console.log(somast);
            ...
            html+='</thead>';
            html+='<tbody>';
           // console.log("raviqty" + qty_bal);
                        
            for (let i = 0; i <somast.length; i++) {
                    const sn_date = moment(somast[i]['so_date']).format('DD/MM/YYYY');
                    var somastdet = somast[i]['sales_or_group'];
                    // else var somastdet='';
                 //console.log(somastdet)
                    for (let j = 0; j < somastdet.length; j++) { 
                        var printallw = true;
                        if ((qty_bal=="true") && parseInt(somastdet[j]['qty_bal'])<=0) printallw=false;
                        if (printallw ==true)
                        {                       
                            
                            
                      
                        // console.log(somastdet[j])
                            html += '<tr>';

                            const sd_date = moment(somast[i]['so_date']).format('DD/MM/YYYY');
                            html += '<td>' + sd_date + '</td>';
                            html += '<td>' + somast[i]['vouc_code'] + '</td>';
                           // console.log("tina"+somast[i]['buy_cus_name']['ac_city']['city_name'])
                            if (somast[i]['buy_cus_name'] != null) html +='<td>'+somast[i]['buy_cus_name']['ac_name'] + '</td>';
                            else html += '<td>' +  '</td>';
                            ...
                            if (somastdet[j]['order_price'] != undefined)  html +='<td>'+somastdet[j]['order_price']+ '</td>';
                            else html += '<td>' +  '</td>';
                            if (somastdet[j]['sopacper_carten'] != undefined) html +='<td>'+somastdet[j]['sopacper_carten']+ '</td>'; 
                            else html += '<td>' +  '</td>';
                            ...
                            if (somastdet[j]['qty_bal'] != undefined) html +='<td>'+somastdet[j]['qty_bal']+ '</td>';  
                            else html += '<td>' +  '</td>';
                            html += '</tr>';
                        }
                    }
...
router.post('/salesorderdetails', ensureAuthenticated, function(req, res){
       var start_date= req.body.start_date;
        var end_date= req.body.end_date;
        var  buy_cus_name=req.body.buy_cus_name;
        var  qtybal=req.body.qty_bal;
        console.log("prabal",qtybal);
        ...
        if (buy_cus_name=="") qry = {$and: [{main_bk:"salesorder"},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
        if (buy_cus_name!="") qry = {$and: [{main_bk:"salesorder"},{buy_cus_name:buy_cus_name},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
        ...
        somast.find(qry,async function (err, somast) {   
                 if (err) {
                       ...
                       // console.log(somast);
                        var html = await CreateHTML(somast,qtybal,buy_cus_name,req);
                        res.json({ 'success': true, 'hhtml':html});
                    }
                }).populate('buy_cus_name').populate([{path: 'sales_or_group.so_disc'}])//,select:'prdt_desc'  

})
router.post('/salesordersummary', ensureAuthenticated, function(req, res){
    var start_date= req.body.start_date;
     var end_date= req.body.end_date;
     var  buy_cus_name=req.body.buy_cus_name;
     var  qty_bal=req.body.qty_bal;
     const strtdate = moment(start_date, "DD/MM/YYYY").tz("Asia/Kolkata").startOf('day').format('x');
     const enddats = moment(end_date, "DD/MM/YYYY").tz("Asia/Kolkata").endOf('day').format('x');
     var qry = "";          
//      if(qty_bal=="true"){
//         if (buy_cus_name=="") qry = {$and: [{main_bk:"salesorder"},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}},{'sales_or_group.qty_bal':{$gt:0}}],co_code:req.session.compid,div_code:req.session.divid};
//         if (buy_cus_name!="") qry = {$and: [{main_bk:"salesorder"},{buy_cus_name:buy_cus_name},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}},{'sales_or_group.qty_bal':{$gt:0}}],co_code:req.session.compid,div_code:req.session.divid};
//    }else{   
      if (buy_cus_name=="") qry = {$and: [{main_bk:"salesorder"},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
      if (buy_cus_name!="") qry = {$and: [{main_bk:"salesorder"},{buy_cus_name:buy_cus_name},{so_datemilisecond:{$gte:strtdate}},{so_datemilisecond:{$lte:enddats}}],co_code:req.session.compid,div_code:req.session.divid};
//    }
//    console.log("dfg"+qry);
       ...
       somast.find(qry,async function (err, somast) {   
              if (err) {
                     ...
                     var html = await CreateHTML1(somast,buy_cus_name,qty_bal,req);
                     res.json({ 'success': true, 'hhtml':html});
                 }
