...
let ptyp_mast= require('../models/subqualitySchema');


// Add Route
router.get('/sub_quality_mast', ensureAuthenticated, function(req, res){
      ...
      //  state_master.find({}, function (err, state_master) {
        ptyp_mast.find({del:"N"}, function (err,ptyp_mast){
            if (err) {
                ...
                res.render('sub_quality_mast.hbs', {
                    pageTitle:'Sub Quality Master',
                    ptyp_mast: ptyp_mast,
...
router.post('/sub_quality_mast_add',function(req, res){
    ...
    else
    {
        ...
        raw.save(function (err){
            if(err)
            {
                res.json({'success':false,'message':'error in saving sub_quality_mast','errors':err});
                return;
            }
            else
            {
                // res.json({ 'success': true, 'message': 'Order added succesfully' });
                res.redirect('/sub_quality_mast/sub_quality_mast');
            }
        });
    }
});
router.get('/:id', ensureAuthenticated, function(req, res){
    ptyp_mast.findById(req.params.id, function(err, ptyp_mast){
        if (err) {
            res.json({ 'success': false, 'message': 'error in fetching sub_quality_mast details' });
        } else {
...
router.post('/edit_sub_quality_mast/:id',function(req, res){
   let errors = req.validationErrors();
    if(errors)
    {
        console.log(errors);
    ...
    else
    {
        ...
        raw.del = 'N';
         ...
         ptyp_mast.update(query, raw, function(err){
            if(err){
                res.json({ 'success': false, 'message': 'Error in Updating sub_quality_mast', errors: err });
            } else {
                res.redirect('/sub_quality_mast/sub_quality_mast');
            }
        });
...
router.delete('/confirm/:id', function(req, res){
    ...
    let query = {_id:req.params.id}
    rawmat.find({Rm_Sub_Qua : req.params.id, del:"N"}, function(err, raw){
            if(err){
              console.log(err);
            }else{
                if(raw != ''){
