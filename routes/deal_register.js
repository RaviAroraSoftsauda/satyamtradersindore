...
// Add Route
router.get('/deal_register', ensureAuthenticated, function(req, res){
             party.find({masterid:req.session.masterid}, function (err, party){
            if (err) {
                console.log(err);
            } else {
                res.render('deal_register.hbs', {
                    pageTitle:'Deal Register',
                    contract: contract,
                    party: party,
        ...
        // {
            ...
            var childarray = [];
                for (let i = 0; i < contract.length; i++) {
                    ...
                    // else html += '<td>' +  '</td>';
                    if (contract[i]['sauda2']['pament_amt'] != undefined) html+='<td><input type="text" name="pament_amt" id="pamentamt'+i+'" value="'+contract[i]['sauda2']['pament_amt']+'" data-pamentamtdsrno="'+i+'" data-id="'+contract[i]['sauda2']['_id']+'" class="form-control pament_amt"autocomplete="off"></td>';
                    else html+='<td><input type="text" name="pament_amt" id="pamentamt'+i+'" data-pamentamtdsrno="'+i+'" data-id="'+contract[i]['sauda2']['_id']+'" class="form-control pament_amt"autocomplete="off"></td>';
...
router.post('/dealrecv_amt/:id', function(req, res) {
    var dealrecvamt = req.body.dealrecvamt;
    let errors = req.validationErrors();
    if (errors) {
        console.log(errors);
        res.json({ 'success': false, 'message': 'validation error', 'errors': errors });
        ...
        let contractsauda = {};     
        contractsauda.dealrecv_amt =dealrecvamt;
        let query = {_id:req.params.id}
        sauda2.update(query ,contractsauda ,function (err) {
            if (err) {
                res.json({ 'success': false, 'message': 'Error in Saving manufctur', 'errors': err });
