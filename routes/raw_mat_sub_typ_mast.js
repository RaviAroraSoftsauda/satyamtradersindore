...
let raw_mast= require('../models/raw_mat_sub_typ_mast');
//let district_master = require('../models/district_schema');
//let state_master = require('../models/state_schema');

// Add Route
router.get('/raw_mat_sub_typ_mast', ensureAuthenticated, function(req, res){
   // district_master.find({}, function (err, district_master){
      //  state_master.find({}, function (err, state_master) {
        raw_mast.find({}, function (err,raw_mast){
            if (err) {
                console.log(err);
            } else {
                res.render('raw_mat_sub_typ_mast.hbs', {
...
router.post('/raw_mat_sub_typ_mast_add',function(req, res){
    ...
    {
        let raw = new raw_mast();
        raw.sub_descr = req.body.sub_descr;
        raw.sub_Code= req.body.sub_Code;
        ...
        raw.save(function (err){
            ...
            else
            {
                ...
                res.redirect('/raw_mat_sub_typ_mast/raw_mat_sub_typ_mast');
            }
        });
    }
});
router.get('/:id', ensureAuthenticated, function(req, res){
    raw_mast.findById(req.params.id, function(err, raw_mast){
        if (err) {
            res.json({ 'success': false, 'message': 'error in fetching brnad details' });
        } else {
            res.json({ 'success': true, 'raw_mast': raw_mast });
        }
        
    });
});
router.post('/edit_raw_sub/:id',function(req, res){
   let errors = req.validationErrors();
    if(errors)
    {
        console.log(errors);
    ...
    else
    {
        ...
        raw.masterid =   req.session.masterid;
         ...
         raw_mast.update(query, raw, function(err){
            if(err){
...
router.delete('/:id', function(req, res){
    if(!req.user._id){
      ...
      let query = {_id:req.params.id}
      raw_mast.findById(req.params.id, function(err, raw_mast){
        raw_mast.remove(query, function(err){
            if(err){
              console.log(err);
            }
            res.send('Success');
