...
router.get('/profarma', ensureAuthenticated, function(req, res){
    promast.find({co_code:req.session.compid,div_code:req.session.divid,main_bk:"proforma"}, function (err, promast){
        party.find({masterid:req.session.masterid}, function (err, party){
           acmast.find({masterid:req.session.masterid,main_bk:"customer"}, function (err, acmast){
                ...
                res.render('profarma_entry.hbs', {
                    pageTitle:'Add PROFARMA Entery',
                    promast: promast,
                    workmast: workmast,
                    brand: brand,
                    party: party,
        ...
        else
        {
           ...
           proforma.save(function (err){
                ...
                else
                {
                    ...
                    res.redirect('/profarma/proforma_list');
    ...
    router.get('/proforma_list', ensureAuthenticated ,function(req,res){
        promast.find({main_bk:"proforma"}, function (err,promast){
            if(err)
            {
                console.log(err);
            }
       ...
       router.get('/proforma_update/:id', ensureAuthenticated, function(req, res){
        promast.findById(req.params.id, function (err, promast){
            workmast.find({masterid:req.session.masterid}, function (err, workmast){
             ...
             if (err) {
                    ...
                    res.render('proforma_update.hbs',{
                        pageTitle:'Update Proforma entry',
                        promast: promast,
                        workmast: workmast,
                        brand: brand,
                        party: party,
...
router.get('/proforma_print/', ensureAuthenticated, function(req, res){
