...
var query;
router.get('/getPartyDetail', function (req, res) {
    accname.findById(req.query.party,function(err, party){
        var count = 0;
            count = count + 1;
            qry = {outstanding_balance:{$gt:0},cashac_name:req.query.party,$or:[{OS_Type:'SB'},{OS_Type:'PB'}],div_code:req.session.divid,co_code:req.session.compid,del:'N'};
            outstanding.find(qry,function(err, accnametyp){
                if(accnametyp == null || accnametyp == '' || accnametyp == []){
                    res.json({ 'success': true, 'accnametyp': accnametyp,'party':party});
                }else{
                    res.json({ 'success': true, 'accnametyp': accnametyp,'party':party})
                }
    ...
    accname.findById(req.query.party,function(err, party){
        if(party.GroupName.MaintainOs == 'Y'){
            ...
            if(party.GroupName.GroupType == 'Assets' && req.query.d_c == 'C'){
                qry = {outstanding_balance:{$gt:0},cashac_name:req.query.party,OS_Type:'SB',div_code:req.session.divid,co_code:req.session.compid,del:'N'};
                outstanding.find(qry,function(err, accnametyp){
...
router.get('/Credit_Note_List', ensureAuthenticated ,function(req,res){
    cashmast.find({main_bk:'CN',d_c:'C',co_code: req.session.compid,div_code:req.session.divid,del:'N',srno:0}, function (err, cashmast){
        if(err)
        {
            console.log(err);
        }
        ...
        {
            res.render('Credit_Note_List.hbs',{
...
router.get('/Credit_Note_Add', ensureAuthenticated, function(req, res){
    vouchMast.find({Module:'Credit Note',Vo_Division: {$elemMatch: {$eq: req.session.divid}},masterid:req.session.masterid}, function(err, vouchMast){
        cashmast.aggregate((
            [{ $match: { main_bk:"CN",c_j_s_p:vouchMast[0].Vo_book,co_code:req.session.compid,div_code:req.session.divid,del:"N" } },
            { $sort: { vouc_code: -1} },
            { $limit :1 },
            { $group:
                {
            ...
            addlessMast.findOne({Module:'Credit Note',masterid:req.session.masterid},function(err, addlessmast){
                if (err) {
                    console.log(err);
                } else {
                    var AddLessArr = [];
                    ...
                    res.render('Credit_Note_Add.hbs', {
                        pageTitle:'Credit Note Entry',
                        lastNo:lastNo,
...
router.post('/add', async function(req, res){
    addlessMast.findOne({Module:'Credit Note',masterid:req.session.masterid},async function(err, addlessmast){
        ...
        else
        {
            ...
            credit.cashac_name          = req.body.Credit_AC;
            credit.cash_bank_name       = req.body.Debit_AC;
            ...
            credit.CNCL                 = 'N';
            cashmast.findOne({main_bk:'CN',vouc_code:req.body.vouc_code,c_j_s_p:req.body.c_j_s_p,del:'N',co_code:req.session.compid,div_code:req.session.divid},function(err,data){
                if(data == null || data == ''){
                    credit.save(async function (err){
                        if(err){
                            ...
                            debit.cashac_name          = req.body.Debit_AC;
                            debit.cash_bank_name       = req.body.Credit_AC;
                                    ...
                                    for(let b=0; b<addlessmast.Add_Less_Parameter_Master_Array.length; b++){
                                        for(let c=0; c<addlessmast.Add_Less_Parameter_Master_Array[b].Division.length; c++){
                                            if(addlessmast.Add_Less_Parameter_Master_Array[b].Division[c] == req.session.divid && Add_Array[a].Parameter_id == addlessmast.Add_Less_Parameter_Master_Array[b]._id){
                                                ...
                                                credit_posting.CrNote_id            = credit._id;
                                                credit_posting.main_bk_posting      = "CN_Posting"+a;
                                                credit_posting.srno                 = 100+parseInt(a);
                                                credit_posting.main_bk              = "CN";
                            ...
                            res.redirect('/Credit_Note/Credit_Note_Add');
                        }
                    });
                }else{
                    res.send("<script>alert('This Entry No. Is Already Exist');window.location.href = '/Credit_Note/Credit_Note_Add'</script>");
                }
            })
...
router.get('/Credit_Note_Update/:id', ensureAuthenticated, function(req, res){
     cashmast.findById(req.params.id, function (err,Credit_Note){
        vouchMast.find({Module:'Credit Note',Vo_Division: {$elemMatch: {$eq: req.session.divid}},masterid:req.session.masterid}, function(err, vouchMast){
            addlessMast.findOne({Module:'Credit Note',masterid:req.session.masterid},function(err, addlessmast){
                outstanding.find({CrNote_id:req.params.id,del:'N'},function(err,OutEntry){
                    if(Credit_Note != null){
                        Group.findById(Credit_Note.cashac_name.GroupName,function(err,group){
                            if (err) {
                                ...
                                addlessmast.Add_Less_Parameter_Master_Array = AddLessArr;
                                res.render('Credit_Note_Update.hbs',{
        ...
        else
        {
            ...
            cashmast.update(qry_credit,credit,async function (err){
                if(err){
                                ...
                                for(let c=0; c<addlessmast.Add_Less_Parameter_Master_Array[b].Division.length; c++){
                                    if(addlessmast.Add_Less_Parameter_Master_Array[b].Division[c] == req.session.divid && Add_Array[a].Parameter_id == addlessmast.Add_Less_Parameter_Master_Array[b]._id){
                                        if(Add_Array[a].id == null || Add_Array[a].id == '' || Add_Array[a].id == undefined){
                                            let credit_posting = new cashmast();
                                            credit_posting.CrNote_id            = req.params.id;
                                            credit_posting.main_bk_posting      = "CN_Posting"+a;
                                            credit_posting.srno                 = 100+parseInt(a);
                                            credit_posting.main_bk              = "CN";
                                            if(Add_Array[a].Parameter_Type == '+')credit_posting.d_c = "C";
                                            ...
                                            credit_posting_Up.CrNote_id            = req.params.id;
                                            credit_posting_Up.main_bk_posting      = "CN_Posting"+a;
                                            credit_posting_Up.srno                 = 100+parseInt(a);
                                            credit_posting_Up.main_bk              = "CN";
                                            ...
                                            var qry_posting = {CrNote_id:req.params.id,main_bk_posting:"CN_Posting"+a};
                    ...
                    res.redirect('/Credit_Note/Credit_Note_List');
                }
            });
        }
    });
});
router.get('/Credit_Note_Print/:id', ensureAuthenticated, function(req, res){
    cashmast.findById(req.params.id, function (err,Credit_Note){
       vouchMast.find({Module:'Credit Note',Vo_Division: {$elemMatch: {$eq: req.session.divid}},masterid:req.session.masterid}, function(err, vouchMast){
           addlessMast.findOne({Module:'Credit Note',masterid:req.session.masterid},function(err, addlessmast){
               outstanding.find({CrNote_id:req.params.id,del:'N'},function(err,OutEntry){
                        ...
                        if(Credit_Note != null){
                            Group.findById(Credit_Note.cashac_name.GroupName,function(err,group){
                                if (err) {
                                    ...
                                    addlessmast.Add_Less_Parameter_Master_Array = AddLessArr;
                                    res.render('Credit_Note_Print.hbs',{
                                        pageTitle:'Credit Note Print',
                                        Credit_Note:Credit_Note,
                                        OutEntry:OutEntry,
                                        group:group,
                                        vouchMast:vouchMast,
    ...
    if(!req.user._id){
      ...
      cashmast.update(query,Garu, function(err,somast){
          if(err){
            ...
            querytrans = {CrNote_id:req.params.id};
            cashmast.updateMany(querytrans,trans,async function(err,trans){
                if(err)res.json({'success': "false"});
                        ...
                        trans.del = 'Y';
                        trans.delete = new Date();
                        var outSR = await outstanding.find({CrNote_id:req.params.id},function(err,aa){});
                        outstanding.updateMany({CrNote_id:req.params.id},trans,function(err){});
